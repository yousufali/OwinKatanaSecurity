...
{
    public class BasicAuthentioncationHandler : AuthenticationHandler<BasicAuthenticationOptions>
    {
        private readonly string m_challenge;

        public BasicAuthentioncationHandler(BasicAuthenticationOptions options)
        {
            m_challenge = "Basic realm=" + options.Realm;
        }
        ...
        protected override Task ApplyResponseChallengeAsync()
        {
            if (Response.StatusCode == 401)
            {
                ...
                if (challenge != null)
                {
                    Response.Headers.AppendValues("WWW-Authenticate", m_challenge);
                }
            }
